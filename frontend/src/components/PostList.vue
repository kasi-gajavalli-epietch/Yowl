...
<div id="container">
	...
	<article class="card"><img :src="`https://api.apiflash.com/v1/urltoimage?access_key=0f9dbd96392046419bb27278589c40d3&url=`+ post.link + '&width=400&height=760'" alt="thumbnail" class="thumbnail">
		<div class="cat-links"><a href="#">{{countCommentsByPost(post)}}<i class="fas fa-comment"></i></a></div>
		<div class="preview" id="box1">
		<h2><a href="#">{{(post.link.length>38) ? post.link.substring(0,38)+".." : post.link}}</a></h2>
			<p>{{post.description}} <a href="#" class="moremore"></a></p>
			<a href="#" id="rm"><router-link :to="{name: 'PostDetail', params: {id: post.id} }">read more</router-link></a>
	</div>
	</article>
    </body>
    </div>
